...
The CSV path, LLM endpoint, context window, chunk limit, output limit, token estimation ratio, and request timeout are all configurable through `.env`. `CHUNK_MAX_TOKENS` plus `MAX_LLM_OUTPUT_TOKENS` must remain below `MAX_CONTEXT_TOKENS`.
